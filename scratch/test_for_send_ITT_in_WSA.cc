...
void ReceivePacket_WSA (Ptr<Socket> socket)
{
 ...
 NS_LOG_INFO ("CLEINT > Source Received " << buf);
}

// // send WSA message with ITT in buffer
// static void SendWSA_ITT (Ptr<Socket> socket, uint32_t pktSize,
//                          uint32_t pktCount, Time pktInterval, uint8_t new_ITT)

// {
//   if (pktCount > 0)
//     {
//       uint8_t buffer = new_ITT;
//       socket->Send (Create<Packet> (buffer, 200));
//       Simulator::Schedule (pktInterval, &SendWSA_ITT,
//                            socket, pktSize,pktCount - 1, pktInterval, new_ITT);
//     }
//   else
//     {
//       socket->Close ();
